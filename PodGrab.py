...
NUM_MAX_DOWNLOADS = 100

DOWNLOAD_DIRECTORY = "podcasts"

# Added 2011-10-06 Werner Avenant - added current_dictory here so it can
# be global
current_directory = ''
m3u_file = ''
...
def main(argv):
    ...
    parser.add_argument(
        '-s', '--subscribe', action="store", dest="sub_feed_url",
    ...
    parser.add_argument('-un', '--unsubscribe', action="store",
                        dest="unsub_url", help='Unsubscribe from the following Podcast feed')
    parser.add_argument(
        '-ma', '--mail-add', action="store", dest="mail_address_add",
                        help='Add a mail address to mail subscription updates to')
    parser.add_argument('-md', '--mail-delete', action="store",
                        dest="mail_address_delete", help='Delete a mail address')

    parser.add_argument('-l', '--list', action="store_const", const="ALL",
                        dest="list_subs", help='Lists current Podcast subscriptions')
    parser.add_argument('-u', '--update', action="store_const", const="UPDATE",
                        dest="update_subs", help='Updates all current Podcast subscriptions')
    parser.add_argument('-ml', '--mail-list', action="store_const",
                        const="MAIL", dest="list_mail", help='Lists all current mail addresses')

    parser.add_argument('-io', '--import', action="store",
                        dest="opml_import", help='Import subscriptions from OPML file')
    parser.add_argument(
        '-eo', '--export', action="store_const", const="OPML_EXPORT",
                        dest="opml_export", help='Export subscriptions to OPML file')
        ...
        if mode == MODE_UNSUBSCRIBE:
            ...
            if feed_name == "None":
                ...
                try:
                    ...
                    print(
                        "Subscription directory has not been found - it might have been manually deleted")
            ...
            for sub in subs:
                ...
                if not data:
                    print("'" + feed_url + "' for '" +
                          feed_name + "' is not a valid feed URL!")
                else:
            ...
            print(
                iterate_feed(data, mode, download_directory, todays_date, cursor, connection, feed_url))
            ...
            import_opml_file(
                cursor, connection, current_directory, download_directory, import_file_name)
    else:
        print("Sorry, there was some sort of error: '" +
              error_string + "'\nExiting...\n")
        if connection:
            connection.close()
...
def export_opml_file(cur, conn, cur_dir):
    ...
    last_ep = ""
    now = datetime.datetime.now()
    file_name = cur_dir + os.sep + "podgrab_subscriptions-" + \
        str(now.year) + "-" + str(now.month) + "-" + str(now.day) + ".opml"
    subs = get_subscriptions(cur, conn)
    file_handle = open(file_name, "w")
    print("Exporting RSS subscriptions database to: '" +
          file_name + "' OPML file...please wait.\n")
    ...
    for sub in subs:
        ...
        feed_url = sub[1]
        last_ep = sub[2]
        file_handle.writelines("\t<outline title=\"" + feed_name + "\" text=\"" + feed_name +
                               "\" type=\"rss\" xmlUrl=\"" + feed_url + "\" htmlUrl=\"" + feed_url + "\"/>\n")
        print("Exporting subscription '" + feed_name + "'...Done.\n")
        item_count = item_count + 1
    footer = "</body>\n</opml>"
    file_handle.writelines(footer)
    file_handle.close()
    print(str(item_count) + " item(s) exported to: '" +
          file_name + "'. COMPLETE")
...
def import_opml_file(cur, conn, cur_dir, download_dir, import_file):
    ...
    if import_file.startswith("/") or import_file.startswith(".."):
        ...
        if not data:
            print("ERROR - Could not open OPML file '" +
                  cur_dir + os.sep + import_file + "'")
    if data:
        ...
        try:
            ...
            for item in items:
                ...
                if not does_sub_exist(cur, conn, item_feed):
                    ...
                    print(
                        "This subscription is already present in the database. Skipping...")
                print("\n")
            print("\nA total of " + str(count) +
                  " subscriptions have been added from OPML file: '" + import_file + "'")
...
def iterate_feed(data, mode, download_dir, today, cur, conn, feed):
    ...
    try:
        ...
        for channel in xml_data.getElementsByTagName('channel'):
            channel_title = channel.getElementsByTagName(
                'title')[0].firstChild.data
            channel_link = channel.getElementsByTagName(
          ...
          # 2011-10-06 Replaced channel_directory with channel_title - needed
          # for m3u file later
                num_podcasts = iterate_channel(
                    channel, today, mode, cur, conn, feed, channel_title)
        ...
        # 2011-10-06 Replaced channel_directory with channel_title - needed for
        # m3u file later
                    num_podcasts = iterate_channel(
                        channel, today, mode, cur, conn, feed, channel_title)
                ...
                num_podcasts = iterate_channel(
                    channel, today, mode, cur, conn, feed, channel_title)
                message += str(num_podcasts) + \
                    " have been downloaded from your subscription: '" + \
                    channel_title + "'\n"
    except xml.parsers.expat.ExpatError:
        print("ERROR - Malformed XML syntax in feed. Skipping...")
...
def write_podcast(item, channel_title, date, type):
    ...
    local_file = current_directory + os.sep + DOWNLOAD_DIRECTORY + \
        os.sep + channel_title + os.sep + clean_string(item_file_name)
    ...
    if os.path.exists(local_file) and os.path.getsize(local_file) != 0:
        ...
        try:
            ...
            # saved to disk - probably a python feature (sorry, only wrote my
            # first line of python today)
            ...
            output.write(DOWNLOAD_DIRECTORY + os.sep +
                         channel_title + os.sep + item_file_name + "\n")
            output.close()
...
def mail_updates(cur, conn, mess, num_updates):
    ...
    for address in addresses:
        try:
            ...
            mail('localhost', 'podgrab@' + platform.node(),
                 address[0], subject_line, mess)
...
def setup_database(cur, conn):
    cur.execute(
        "CREATE TABLE subscriptions (channel text, feed text, last_ep text)")
...
def insert_subscription(cur, conn, chan, feed):
    ...
    row = (chan, feed, "Thu, 01 Jan 1970 00:00:00")
           # Added a correctly formatted date here so we can avoid an ugly "if
           # date == null" in update_subscription later
    cur.execute(
        'INSERT INTO subscriptions(channel, feed, last_ep) VALUES (?, ?, ?)', row)
    conn.commit()
...
def iterate_channel(chan, today, mode, cur, conn, feed, channel_title):
    ...
    if does_sub_exist(cur, conn, feed):
        ...
        insert_subscription(
            cur, conn, chan.getElementsByTagName('title')[0].firstChild.data, feed)
    ...
    for item in chan.getElementsByTagName('item'):
        try:
            ...
            item_file = item.getElementsByTagName(
                'enclosure')[0].getAttribute('url')
            item_size = item.getElementsByTagName(
                'enclosure')[0].getAttribute('length')
            item_type = item.getElementsByTagName(
                'enclosure')[0].getAttribute('type')
            struct_time_today = strptime(today, "%a, %d %b %Y %H:%M:%S")

            try:
                struct_time_item = strptime(
                    fix_date(item_date), "%a, %d %b %Y %H:%M:%S")
                has_error = 0
            except TypeError:
                has_error = 1
            except ValueError:
            ...
            if not has_error:
                if mktime(struct_time_item) > mktime(struct_last_ep) or mode == MODE_DOWNLOAD:
                    saved = write_podcast(
                        item_file, channel_title, item_date, item_type)

                    if saved == 'File Exists':
                        print(
                            "File Existed - updating local database's Last Episode")
                        update_subscription(
                            cur, conn, feed, fix_date(item_date))
                    ...
                    print(
                        "According to database we already have the episode dated " + item_date)
                    break
...
def update_subscription(cur, conn, feed, date):
    ...
    # Presumes that "null" dates will be saved in DB as 1970-01-01 (unix
    # "start" time)
